...
// subBalanceWrapper is used to keep the configurations that will be used to start
...
type subBalanceWrapper struct {
	// subBalanceWrapper is passed to the sub-balancer as a ClientConn
...
func (sbc *subBalanceWrapper) UpdateState(state balancer.State) {
	sbc.mu.Lock()
	sbc.state = state
...
func (sbc *subBalanceWrapper) NewSubConn(addrs []resolver.Address, opts balancer.NewSubConnOptions) (balancer.SubConn, error) {
	return sbc.group.newSubConn(sbc, addrs, opts)
}

func (sbc *subBalanceWrapper) updateBalancerStateWithCachedPicker() {
	sbc.mu.Lock()
...
func (sbc *subBalanceWrapper) startBalancer() {
...
func (sbc *subBalanceWrapper) updateSubConnState(sc balancer.SubConn, state balancer.SubConnState) {
	b := sbc.balancer
	if b == nil {
...
func (sbc *subBalanceWrapper) updateClientConnState(s balancer.ClientConnState) error {
	sbc.ccState = &s
	b := sbc.balancer
	if b == nil {
...
func (sbc *subBalanceWrapper) resolverError(err error) {
	b := sbc.balancer
	if b == nil {
		// This sub-balancer was closed. This should never happen because
		// sub-balancers are closed when the locality is removed from EDS, or
...
func (sbc *subBalanceWrapper) stopBalancer() {
	sbc.balancer.Close()
	sbc.balancer = nil
}
...
type BalancerGroup struct {
	...
	idToBalancerConfig map[internal.LocalityID]*subBalanceWrapper
	...
	scToSubBalancer map[balancer.SubConn]*subBalanceWrapper
...
func New(cc balancer.ClientConn, loadStore lrs.Store, logger *grpclog.PrefixLogger) *BalancerGroup {
	return &BalancerGroup{
		...
		idToBalancerConfig: make(map[internal.LocalityID]*subBalanceWrapper),
		balancerCache:      cache.NewTimeoutCache(DefaultSubBalancerCloseTimeout),
		scToSubBalancer:    make(map[balancer.SubConn]*subBalanceWrapper),
...
func (bg *BalancerGroup) cleanupSubConns(config *subBalanceWrapper) {
	bg.incomingMu.Lock()
...
// wrapper function.
func (bg *BalancerGroup) newSubConn(config *subBalanceWrapper, addrs []resolver.Address, opts balancer.NewSubConnOptions) (balancer.SubConn, error) {
